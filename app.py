...
st.title("✉️ Envío by Ina - Generador de Diapositivas")
st.write(
    "Cruza tus bases de contactos contra Mailtrap y genera automáticamente la **diapositiva completa en SVG** adaptada al título de tu campaña."
)
...
def obtener_estilo_dinamico(nombre_hoja):
    """
    Asigna el color del subtítulo:
    - Si contiene 'rojo', 'amarillo' o 'verde', usa sus respectivos colores de morosidad.
    - Si NO tiene ninguno de esos tres, el título siempre será NARANJA (#FF5100).
    """
    nombre_limpio = nombre_hoja.strip()
    nombre_lower = nombre_limpio.lower()

    if "rojo" in nombre_lower:
        return {"titulo": f"Morosidad {nombre_limpio}", "color_sub": "#D92D20"}
    elif "amarillo" in nombre_lower:
        return {"titulo": f"Morosidad {nombre_limpio}", "color_sub": "#E08A00"}
    elif "verde" in nombre_lower:
        return {"titulo": f"Morosidad {nombre_limpio}", "color_sub": "#1B9E48"}
    else:
        # Si no es rojo, amarillo ni verde -> Siempre Naranja (#FF5100)
        return {"titulo": nombre_limpio, "color_sub": "#FF5100"}


def generar_diapositiva_svg_completa(
    hoja, total_base, dup, bad, vac, sa, ab, cl, reb, nc
):
    """Genera el SVG completo (1920x1080) usando tipografía Geist, esquinas redondeadas y lógica de color estricta."""
    estilo = obtener_estilo_dinamico(hoja)
    ...
    bar_w = 734
    h_bar = 20.18
    rx_bar = 10.09  # Esquinas 100% redondeadas (round corner)
  ...
  <text x="100" y="195" class="tit-sub">{titulo_sub}</text>
